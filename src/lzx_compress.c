 ...
 * hash table for finding length 2 matches.  This can be as high as 16 (in which
 * case the hash function is trivial), but using a smaller hash table speeds up
 * compression due to reduced cache pressure.
 */
#define BT_MATCHFINDER_HASH2_ORDER	12

/*
 * Compressor-side limits on the codewords lengths, in bits, for each Huffman
 * code.  To make outputting the bits faster, these values are slightly lower
 * than the format-defined limits.
 */
...
struct lzx_sequence {
	...
	/* The match length minus LZX_MIN_MATCH_LEN  */
	u16 adjusted_length;

	/*
	 * This contains either:
	 *
	 *	Bits 0-8: The match header, i.e. the offset slot and length
	 *		  header packed together.
	 *	Bits 9-31: The match offset minus LZX_OFFSET_ADJUSTMENT.
	 *
	 * or:
	 *	the special value 0xFFFFFFFF to indicate that this was the last
	 *	literal run in this block.
	 */
	u32 adjusted_offset__and_match_hdr;
};
...
struct lzx_compressor {
	...
	/* The matches and literals chosen for the current block.  */
	struct lzx_sequence chosen_sequences[LZX_DIV_BLOCK_SIZE / LZX_MIN_MATCH_LEN + 1];
...
/*
 * Call a hc_matchfinder function.
 */
...
/*
 * Call a bt_matchfinder function.
 */
		...
		const struct lzx_codes *codes)
{
	...
	for (;;) {
		...
		if (seq->adjusted_offset__and_match_hdr & 0x80000000)
			return;

		match_hdr = seq->adjusted_offset__and_match_hdr & 0x1FF;
		...
		adjusted_offset = seq->adjusted_offset__and_match_hdr >> 9;
			   ...
			   struct lzx_output_bitstream * os)
{
	...
	lzx_write_items(os, block_type, block_begin, sequences, codes);
}
 ...
 */
static inline void
lzx_record_literal(struct lzx_compressor *c, unsigned literal,
		   u32 *litrunlen_p)
{
 ...
 * offsets queue.  */
static inline void
lzx_record_match(struct lzx_compressor *c,
		 unsigned length, u32 offset_data,
		 ...
		 u32 *litrunlen_p, struct lzx_sequence **next_seq_p)
{
	...
	next_seq->adjusted_offset__and_match_hdr = (offset_data << 9) | v;
...
lzx_finish_sequence(struct lzx_sequence *last_seq, u32 litrunlen)
{
	...
	last_seq->adjusted_offset__and_match_hdr = 0xFFFFFFFF;
}

/* Trace the minimum-cost backwards and increment the frequencies of the symbols
 * that need to be used to represent the matches and literals in this path.  */
static inline void
...
/* Trace the minimum-cost backwards, increment the frequencies of the symbols
 * that need to be used to represent the matches and literals in this path, and
 * save an intermediate representation of the matches and literals
 * 'c->chosen_sequences'.  */
static inline u32
lzx_record_item_list(struct lzx_compressor *c, u32 block_size, bool is_16_bit)
{
	...
	c->chosen_sequences[seq_idx].adjusted_offset__and_match_hdr = 0xFFFFFFFF;
	...
	for (;;) {
		...
		/* Save the literal run length for the previous sequence.  */
		...
		/* Save the adjusted offset and match header.  */
		c->chosen_sequences[seq_idx].adjusted_offset__and_match_hdr =
				(offset_data << 9) | v;
...
out:
	...
	/* Return the index of the first lzx_sequence in c->chosen_sequences
	 * that is used.  */
	return seq_idx;
}
			     ...
			     bool is_16_bit)
{
	...
	item_idx = lzx_record_item_list(c, block_size, is_16_bit);
