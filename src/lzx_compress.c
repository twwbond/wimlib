...
struct lzx_sequence {
	...
	/* The match header, or the special value 0xFF to indicate that this was
	 * the last literal run in this block.  The match header contains the
	 * offset slot and length header packed together  */
	u8 match_hdr;

	/* The match length minus LZX_MIN_MATCH_LEN  */
	u8 adjusted_length;

	/* The match offset minus LZX_OFFSET_ADJUSTMENT  */
	u32 adjusted_offset;
};
		 ...
		 u32 *litrunlen_p, struct lzx_sequence **next_seq_p)
{
	...
	/* Save the literal run length, adjusted length, and adjusted offset  */
	next_seq->litrunlen = litrunlen;
	next_seq->adjusted_length = v;
	next_seq->adjusted_offset = offset_data;
	...
	/* Save the match header.  */
	next_seq->match_hdr = v;
...
lzx_finish_sequence(struct lzx_sequence *last_seq, u32 litrunlen)
{
	...
	/* Special match header to mark last sequence  */
	last_seq->match_hdr = 0xFF;
}
...
{
	u32 node_idx = block_size;
	u32 item_idx = ARRAY_LEN(c->chosen_sequences) - 1;

	c->chosen_sequences[item_idx].match_hdr = 0xFF;
	...
	for (;;) {
		...
		unsigned v = len - LZX_MIN_MATCH_LEN;;
		unsigned offset_slot = lzx_comp_get_offset_slot(c, offset_data, is_16_bit);
		c->chosen_sequences[item_idx].litrunlen = lit_start_node - node_idx;
		node_idx -= len;
		item_idx--;
		c->chosen_sequences[item_idx].adjusted_length = v;
		c->chosen_sequences[item_idx].adjusted_offset = offset_data;
		...
		v += offset_slot * LZX_NUM_LEN_HEADERS;
		c->freqs.main[LZX_NUM_CHARS + v]++;
		c->chosen_sequences[item_idx].match_hdr = v;
		if (node_idx == 0)
			goto out;
	}
			     ...
			     bool is_16_bit)
{
	...
	do {
		new_queue = lzx_find_min_cost_path(c, block_begin, block_size, initial_queue,
						   is_16_bit);
