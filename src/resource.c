 ...
 *
 * We currently support three variations on this resource format, orthogonal to
 * the compression type and chunk size which can vary as well:
 *
 ...
 *   the resource begins with a 16-byte header specifies the uncompressed size
 *   of the resource, the compression type, and the chunk size.  (In the
 ...
 *   file have this format): similar to the original format, but the chunk table
 *   is at the end of the resource rather than the beginning, and each
 *   compressed chunk is prefixed with its compressed size as a 32-bit integer.
 *   This format allows a resource to be written without rewinding.
 */
 ...
 * @cbs
 *	The consume_chunk() callback to feed the data being read.  Each call
 *	provides the next chunk of the requested data, uncompressed.  Each chunk
 *	will be nonempty and will not cross range boundaries but otherwise will
 *	be of unspecified size.
 *
 ...
 *
 *	or other error code returned by the consume_chunk() function.
 */
static int
...
/* A consume_chunk() implementation that simply concatenates all chunks into a
 * buffer.  */
static int
 ...
 *
 * @size and @offset are assumed to have already been validated against the
...
int
read_partial_wim_blob_into_buf(const struct blob_descriptor *blob, u64 offset,
...
/* Convert a WIM resource header from the on-disk format into an in-memory
 * format.  */
void
...
/* Convert a WIM resource header from an in-memory format into the on-disk
 * format.  */
void
