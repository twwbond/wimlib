		     ...
		     const struct ntfs_3g_apply_ctx *ctx)
{
	...
	{
		struct SECURITY_CONTEXT sctx = {.vol = ni->vol };
		const void *desc = sd->descriptors[inode->i_security_id];
		size_t desc_size = sd->sizes[inode->i_security_id];

		if (ntfs_set_ntfs_acl(&sctx, ni, desc, desc_size, 0)) {
			if (wimlib_print_errors) {
				...
				if (errno_saved == EINVAL) {
					...
					print_byte_field(desc, desc_size,
							 wimlib_error_file);
					fprintf(wimlib_error_file, "\n"
