...
/* Read the header for a blob in a pipable WIM.  If @pwm_hdr_okay is true, also
 * look for a pipable WIM header and return PWM_FOUND_WIM_HDR if found.  */
static int
read_pwm_blob_header(WIMStruct *pwm, bool pwm_hdr_okay,
		     ...
		     struct wim_header_disk *pwm_hdr_ret)
{
	...
	if (pwm_hdr_okay && le64_to_cpu(blob_hdr.magic) == PWM_MAGIC) {
		u8 *p = (u8 *)pwm_hdr_ret;
		p = mempcpy(p, &blob_hdr, sizeof(blob_hdr));
		ret = full_read(&pwm->in_fd, p,
	...
	if (unlikely(le64_to_cpu(blob_hdr.magic) != PWM_BLOB_MAGIC)) {
	...
	if (unlikely(!reshdr_ret->uncompressed_size)) {
		     ...
		     const struct read_blob_list_callbacks *cbs)
{
	...
	while (ctx->num_blobs_remaining) {
		...
		ret = read_pwm_blob_header(ctx->wim, true, hash, &reshdr, &pwm_hdr);
					     ...
					     void *progctx)
{
	 ...
	 * WIMs.)  */
	{
		...
		ret = read_pwm_blob_header(pwm, false, hash,
					   &pwm->hdr.xml_data_reshdr, &pwm_hdr);
		if (ret)
			goto out_wimlib_free;
	...
	for (i = 1; i <= pwm->hdr.image_count; i++) {
		...
		ret = read_pwm_blob_header(pwm, false, imd->metadata_blob->hash,
					   &reshdr, &pwm_hdr);
		if (ret)
			goto out_wimlib_free;
