 ...
 */
static int
add_stream(struct wim_inode *inode, int stream_type,
	   ...
	   struct list_head *unhashed_blobs)
{
	...
	if (unlikely(strm)) {
		...
		if (stream_blob_resolved(strm))
			WARNING("Inode has multiple nonempty unnamed "
				"data streams!  Only one will be saved");
	       ...
	       const ATTR_RECORD *record)
{
	...
	ret = add_stream(inode, attr_type_to_wimlib_stream_type(type),
